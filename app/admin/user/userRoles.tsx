    ...
    <div className="p-8 bg-[#0a0a0a] min-h-screen text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-[#1a1a1a] p-5 rounded-2xl border border-zinc-800 flex items-center gap-4">
          ...
          <div><p className="text-zinc-500 text-sm">Admins</p><h3 className="text-xl font-bold">1</h3></div>
        </div>
        <div className="bg-[#1a1a1a] p-5 rounded-2xl border border-zinc-800 flex items-center gap-4">
          <div className="p-3 bg-green-500/10 rounded-xl text-green-500"><User size={24} /></div>
          <div><p className="text-zinc-500 text-sm">Standard Users</p><h3 className="text-xl font-bold">2</h3></div>
        </div>
      </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-zinc-900/50 text-zinc-400 text-sm uppercase">
            <tr>
              ...
              <th className="p-4 font-semibold text-center">Last Login / History</th>
              <th className="p-4 font-semibold text-center">Status</th>
              <th className="p-4 font-semibold text-right text-center">Actions</th>
            </tr>
          </thead>
              ...
              <tr key={user.id} className="hover:bg-zinc-800/30 transition-colors">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-1.5">
                    <div className={`w-2 h-2 rounded-full ${user.status === 'Active' ? 'bg-green-500' : 'bg-zinc-600'}`}></div>
                    <span className="text-sm text-zinc-300">{user.status}</span>
                  </div>
                </td>
                <td className="p-4">
                  <div className="flex justify-end gap-2">
                    <button className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-400 transition-colors" title="View History">
                      <Eye size={18} />
                    </button>
